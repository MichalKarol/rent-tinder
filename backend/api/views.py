...
class NotSeenRentOffersViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        user_reactions = Reaction.objects.filter(user=self.request.user, reaction__in=[-1, 1]).values('rent_id')
...
class ReactionsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return RentOffer.objects.filter(reaction__user=self.request.user).annotate(
            reaction_value=Max('reaction__reaction'),
            max_date=Max('reaction__date')
        ).all()
    
    def create(self, request, *args, **kwargs):
        Reaction.objects.create(rent_id_id=int(request.data.get("id")), user=self.request.user, reaction=int(request.data.get("reaction")))
        return Response(status=204)
    
    def partial_update(self, request, *args, **kwargs):
        reaction = Reaction.objects.filter(rent_id_id=int(request.data.get("id")), user=self.request.user).first()
        if not reaction:
...
def register(request):
    ...
    if not user:
        return Response({'error': 'Please provide better password'},
                        status=HTTP_400_BAD_REQUEST)
    return login(request, username, password)
